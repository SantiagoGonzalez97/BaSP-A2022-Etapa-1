...
var list;
list = ['bread', 'milk', 'cereal', 'fruits', 'snacks'];
for (var i = 0; i <=4; i++){
    alert('Exercise 5.a): ' + list[i]);
}

//b) Al array anterior convertir la primera letra de cada palabra en mayúscula y mostrar una alerta por cada palabra modificada.
for (var i = 0; i < list.length; i++) {
    var myList;
    myList = list[i].substring(0,1).toUpperCase() + list[i].substring(1).toLowerCase();
    alert('Exercise 5.b): ' + myList);
}
...
for (var i = 0; i <= 9; i++){
    myArray.push(i)
    console.log('Exercise 5.b): ' + myArray);
}
